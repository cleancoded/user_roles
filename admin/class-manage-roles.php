...
final class Manage_Roles {
	...
	public function add_admin_page() {
		...
		// Get the page title.
		$title = esc_html__( 'Roles', 'user_roles' );

		if ( isset( $_GET['action'] ) && 'edit' === $_GET['action'] && isset( $_GET['role'] ) )
			$title = esc_html__( 'Edit Role', 'user_roles' );

		// Create the Manage Roles page.
		$this->page = add_submenu_page( 'users.php', $title, esc_html__( 'Roles', 'user_roles' ), $edit_roles_cap, 'roles', array( $this, 'page' ) );
