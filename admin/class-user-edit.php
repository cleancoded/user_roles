...
final class User_Edit {
	...
	public function profile_fields( $user ) {
		...
		<h2><?php esc_html_e( 'Roles', 'user_roles' ); ?></h2>
				...
				<td>
					<div class="wp-tab-panel">
						...
						<?php foreach ( $roles as $role ) : ?>
							...
							<li>
								<label>
									<input type="checkbox" name="user_roles_user_roles[]" value="<?php echo esc_attr( $role->name ); ?>" <?php checked( in_array( $role->name, $user_roles ) ); ?> />
	...
	public function role_update( $user_id, $old_user_data ) {
		...
		if ( ! empty( $_POST['user_roles_user_roles'] ) ) {
			...
			foreach ( $new_roles as $new_role ) {
				...
				if ( user_roles_is_role_editable( $new_role ) && ! in_array( $new_role, (array) $old_user_data->roles ) )
