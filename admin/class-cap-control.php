...
final class Cap_Control {
	...
	public function to_json() {
		...
		$this->json['label'] = array(
			'cap'   => user_roles_show_human_caps() && user_roles_cap_exists( $this->cap ) ? user_roles_get_cap( $this->cap )->label : $this->cap,
			'grant' => sprintf( esc_html__( 'Grant %s capability', 'user_roles' ), "<code>{$this->cap}</code>" ),
			'deny'  => sprintf( esc_html__( 'Deny %s capability',  'user_roles' ), "<code>{$this->cap}</code>" )
		);
