...
final class User_New {
	...
	private function setup_actions() {
		...
		// @link https://github.com/justintadlock/user_roles/issues/153
		if ( ! user_roles_multiple_user_roles_enabled() || is_multisite() )
			return;
	...
	public function profile_fields() {
		...
		$new_user_roles = apply_filters( 'user_roles_default_user_roles', array( get_option( 'default_role' ) ) );

		// If the form was submitted but didn't go through, get the posted roles.
		if ( isset( $_POST['createuser'] ) && ! empty( $_POST['user_roles_user_roles'] ) )
			$new_user_roles = array_map( 'user_roles_sanitize_role', $_POST['user_roles_user_roles'] );

		$roles = user_roles_get_roles();

		ksort( $roles );

		wp_nonce_field( 'new_user_roles', 'user_roles_new_user_roles_nonce' ); ?>

		<table class="form-table">

			<tr>
				<th><?php esc_html_e( 'User Roles', 'user_roles' ); ?></th>
				...
				<td>
					<div class="wp-tab-panel">
						...
						<?php foreach ( $roles as $role ) : ?>
							...
							<?php if ( user_roles_is_role_editable( $role->name ) ) :?>
							<li>
								<label>
									<input type="checkbox" name="user_roles_user_roles[]" value="<?php echo esc_attr( $role->name ); ?>" <?php checked( in_array( $role->name, $new_user_roles ) ); ?> />
	...
	public function user_register( $user_id ) {
		...
		if ( ! isset( $_POST['user_roles_new_user_roles_nonce'] ) || ! wp_verify_nonce( $_POST['user_roles_new_user_roles_nonce'], 'new_user_roles' ) )
			return;
		...
		if ( ! empty( $_POST['user_roles_user_roles'] ) ) {

			// Get the current user roles.
			$old_roles = (array) $user->roles;

			// Sanitize the posted roles.
			$new_roles = array_map( 'user_roles_sanitize_role', $_POST['user_roles_user_roles'] );

			// Loop through the posted roles.
			foreach ( $new_roles as $new_role ) {

				// If the user doesn't already have the role, add it.
				if ( user_roles_is_role_editable( $new_role ) && ! in_array( $new_role, (array) $user->roles ) )
			...
			foreach ( $old_roles as $old_role ) {
				...
				if ( user_roles_is_role_editable( $old_role ) && ! in_array( $old_role, $new_roles ) )
		...
		// If the posted roles are empty.
			...
			foreach ( (array) $user->roles as $old_role ) {
				...
				if ( user_roles_is_role_editable( $old_role ) )
					$user->remove_role( $old_role );
			}
		}
	}
