...
final class Settings_Page {
	 ...
	 * @var    string
	 */
	public $name = 'user_roles-settings';
	...
	private function includes() {
		...
		require_once( user_roles_plugin()->dir . 'admin/functions-settings.php' );

		// Load settings view classes.
		require_once( user_roles_plugin()->dir . 'admin/views/class-view.php'         );
		require_once( user_roles_plugin()->dir . 'admin/views/class-view-general.php' );
		require_once( user_roles_plugin()->dir . 'admin/views/class-view-addons.php'  );
		require_once( user_roles_plugin()->dir . 'admin/views/class-view-donate.php'  );
	}
	...
	public function admin_menu() {
		...
		$this->settings_page = add_options_page(
			esc_html_x( 'user_roles', 'admin screen', 'user_roles' ),
			esc_html_x( 'user_roles', 'admin screen', 'user_roles' ),
			apply_filters( 'user_roles_settings_capability', 'manage_options' ),
			$this->name,
		...
		if ( $this->settings_page ) {
			...
			do_action( 'user_roles_register_settings_views', $this );

			uasort( $this->views, 'user_roles_priority_sort' );
	...
	public function load() {
		...
		$view = $this->get_view( user_roles_get_current_settings_view() );
	...
	public function print_styles() { ?>
		...
		<style type="text/css">
			.settings_page_user_roles-settings .wp-filter { margin-bottom: 15px; }
		</style>
	<?php }
	...
	public function settings_page() { ?>
		...
		<div class="wrap">
			<h1><?php echo esc_html_x( 'user_roles', 'admin screen', 'user_roles' ); ?></h1>

			<div class="wp-filter">
				<?php $this->filter_links(); ?>
			</div>

			<?php $this->get_view( user_roles_get_current_settings_view() )->template(); ?>
	...
	private function filter_links() { ?>
		...
		<ul class="filter-links">
			...
			<?php foreach ( $this->views as $view ) :
				...
				$class = $view->name === user_roles_get_current_settings_view() ? 'class="current"' : '';

				// Get the URL.
				$url = user_roles_get_settings_view_url( $view->name );
