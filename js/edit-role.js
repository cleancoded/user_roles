jQuery( document ).ready( function() {
	...
	jQuery( '.user_roles-delete-role-link' ).click(
		function() {
			return window.confirm( user_roles_i18n.ays_delete_role );
		}
	);
	 ...
	 * @return void
	 */
	function user_roles_print_role_slug( slug ) {
	...
	jQuery( 'input[name="role_name"]' ).keyup(
		function() {
			...
			if ( ! jQuery( 'input[name="role"]' ).val() )
				user_roles_print_role_slug( this.value );
		}
	); // .keyup
	...
	jQuery( document ).on( 'click', '.role-edit-button.closed',
		function() {
			...
			jQuery( this ).removeClass( 'closed' ).addClass( 'open' ).text( user_roles_i18n.button_role_ok );
	...
	jQuery( document ).on( 'click', '.role-edit-button.open',
		function() {
			...
			// Toggle the button class and change the text.
			jQuery( this ).removeClass( 'open' ).addClass( 'closed' ).text( user_roles_i18n.button_role_edit );
			...
			if ( role )
				user_roles_print_role_slug( role );

			// Else, use the role name input value.
			else
				user_roles_print_role_slug( jQuery( 'input[name="role_name"]' ).val() );
		}
	); // .click()
	...
	var section_template = wp.template( 'user_roles-cap-section' );
	var control_template = wp.template( 'user_roles-cap-control' );

	// Check that the `user_roles_sections` and `user_roles_controls` variables were
	// passed in via `wp_localize_script()`.
	if ( typeof user_roles_sections !== 'undefined' && typeof user_roles_controls !== 'undefined' ) {

		// Loop through the sections and append the template for each.
		_.each( user_roles_sections, function( data ) {
			jQuery( '.user_roles-tab-wrap' ).append( section_template( data ) );
		} );

		// Loop through the controls and append the template for each.
		_.each( user_roles_controls, function( data ) {
			jQuery( '#user_roles-tab-' + data.section + ' tbody' ).append( control_template( data ) );
		} );
	}
	...
	jQuery( '.user_roles-cap-tabs .user_roles-tab-content' ).hide();

	// Shows the first tab's content.
	jQuery( '.user_roles-cap-tabs .user_roles-tab-content:first-child' ).show();
	...
	jQuery( '.user_roles-which-tab' ).text( jQuery( '.user_roles-tab-nav :first-child a' ).text() );
	...
	jQuery( '.user_roles-tab-nav li a' ).click(
		function( j ) {
			...
			jQuery( this ).parents( '.user_roles-cap-tabs' ).find( '.user_roles-tab-content' ).hide();

			// Find the tab content that matches the tab nav item and show it.
			jQuery( this ).parents( '.user_roles-cap-tabs' ).find( href ).show();

			// Set the `aria-selected` attribute to false for all tab nav items.
			jQuery( this ).parents( '.user_roles-cap-tabs' ).find( '.user_roles-tab-title' ).attr( 'aria-selected', 'false' );
			...
			jQuery( '.user_roles-which-tab' ).text( jQuery( this ).text() );
		}
	); // click()
	 ...
	 * @return void
	 */
	function user_roles_count_caps() {

		// Count the granted and denied caps that are checked.
		var granted_count = jQuery( "#user_roles-tab-all input[data-grant-cap]:checked" ).length;
		var denied_count  = jQuery( "#user_roles-tab-all input[data-deny-cap]:checked" ).length;

		// Count the new (added from new cap meta box) granted and denied caps that are checked.
		var new_granted_count = jQuery( '#user_roles-tab-custom input[name="grant-new-caps[]"]:checked' ).length;
		var new_denied_count  = jQuery( '#user_roles-tab-custom input[name="deny-new-caps[]"]:checked' ).length;
	 ...
	 * @return void
	 */
	function user_roles_check_uncheck( checkbox ) {
	...
	jQuery( document ).on( 'change',
		'.user_roles-cap-checklist input[data-grant-cap], .user_roles-cap-checklist input[data-deny-cap]',
		function() {

			// Check/Uncheck boxes.
			user_roles_check_uncheck( this );
	...
	jQuery( document ).on( 'click', '.editable-role .user_roles-cap-checklist button',
		function() {

			// Get the button parent element.
			var parent = jQuery( this ).closest( '.user_roles-cap-checklist' );
	...
	jQuery( document ).on( 'hover', '.editable-role .user_roles-cap-checklist button',
		function() {
			jQuery( '.user_roles-cap-checklist button:focus' ).not( this ).blur();
		}
	);
	...
	jQuery( '#user_roles-add-new-cap' ).prop( 'disabled', true );

	// When the user starts typing a new cap.
	jQuery( '#user_roles-new-cap-field' ).on( 'input',
		function() {

			// If there's a value in the input, enable the add new button.
			//if ( 'do_not_allow' !== jQuery( this ).val() ) {
			if ( -1 === jQuery.inArray( jQuery( this ).val(), user_roles_i18n.hidden_caps ) ) {

				jQuery( '#user_roles-add-new-cap' ).prop( 'disabled', false );
	...
	jQuery( '#user_roles-new-cap-field' ).keypress(
		function( e ) {

			// 13 is the key code for "Enter".
			if ( 13 === e.keyCode ) {
				jQuery( '#user_roles-add-new-cap' ).click();
				e.preventDefault();
				return false;
			}
		}
	...
	jQuery( '#user_roles-add-new-cap' ).click(
		function() {
			...
			var new_cap = jQuery( '#user_roles-new-cap-field' ).val();
			...
			if ( new_cap ) {
				...
				jQuery( 'a[href="#user_roles-tab-custom"]' ).trigger( 'click' );

				// Replace text placeholder with cap.
				user_roles_i18n.label_grant_cap = user_roles_i18n.label_grant_cap.replace( /%s/g, '<code>' + new_cap + '</code>' );
				user_roles_i18n.label_deny_cap  = user_roles_i18n.label_deny_cap.replace( /%s/g,  '<code>' + new_cap + '</code>' );
				...
				var data = {
					...
					is_granted_cap : true,
					is_denied_cap  : false,
					label          : { cap : new_cap, grant : user_roles_i18n.label_grant_cap, deny : user_roles_i18n.label_deny_cap }
				};

				// Prepend our template to the "custom" edit caps tab content.
				jQuery( '#user_roles-tab-custom tbody' ).prepend( control_template( data ) );

				// Get the new cap table row.
				var parent = jQuery( '[data-grant-cap="' + new_cap + '"]' ).parents( '.user_roles-cap-checklist' );

				// Add the highlight class.
				jQuery( parent ).addClass( 'user_roles-highlight' );

				// Remove the class after a set time for a highlight effect.
				setTimeout( function() {
					jQuery( parent ).removeClass( 'user_roles-highlight' );
				}, 500 );

				// Set the new cap input value to an empty string.
				jQuery( '#user_roles-new-cap-field' ).val( '' );

				// Disable the add new cap button.
				jQuery( '#user_roles-add-new-cap' ).prop( 'disabled', true );

				// Trigger a change on our new grant cap checkbox.
				jQuery( '.user_roles-cap-checklist input[data-grant-cap="' + new_cap + '"]' ).trigger( 'change' );
			}
		}
	); // .click()
