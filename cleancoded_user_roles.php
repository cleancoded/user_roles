 ...
 * Plugin URI:  https://github.com/cleancoded/user_roles
 * Description: A user and role management plugin that puts you in full control of your site's permissions.
 * Version:     1.0
 * Author:      Cleancoded
 * Author URI:  https://Cleancoded.com
 * Text Domain: user_roles
 * Domain Path: /lang
 */
...
final class user_roles_Plugin {
	 ...
	 *
	 * @see    user_roles_get_role_user_count()
	 * @since  1.0.0
	 * @access public
	 * @var    array
	 */
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Whoah, partner!', 'user_roles' ), '1.0.0' );
	}
	...
	public function __call( $method = '', $args = array() ) {
		_doing_it_wrong( "user_roles_Plugin::{$method}", esc_html__( 'Method does not exist.', 'user_roles' ), '1.0.0' );
		unset( $method, $args );
		return null;
	}
	...
	public function i18n() {
		...
		load_plugin_textdomain( 'user_roles', false, trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) . 'lang' );
	}
	...
	private function get_min_php_message() {
		...
		return sprintf(
			__( 'user_roles requires PHP version %1$s. You are running version %2$s. Please upgrade and try again.', 'user_roles' ),
			$this->php_version,
			PHP_VERSION
		);
	}
...
/**
 * Gets the instance of the `user_roles_Plugin` class.  This function is useful for quickly grabbing data
 ...
 * @return object
 */
function user_roles_plugin() {
	return user_roles_Plugin::get_instance();
}
