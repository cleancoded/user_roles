...
class Role {
	...
	public function __construct( $name, $args = array() ) {
		...
		$this->name = user_roles_sanitize_role( $name );

		if ( $this->caps ) {

			// Validate cap values as booleans in case they are stored as strings.
			$this->caps = array_map( 'user_roles_validate_boolean', $this->caps );
			...
			$this->granted_caps = user_roles_remove_old_levels( $this->granted_caps );
			$this->denied_caps  = user_roles_remove_old_levels( $this->denied_caps  );

			// Remove hidden caps from granted/denied caps.
			$this->granted_caps = user_roles_remove_hidden_caps( $this->granted_caps );
			$this->denied_caps  = user_roles_remove_hidden_caps( $this->denied_caps  );
	...
	public function get( $property ) {
		...
		if ( 'label' === $property )
			return user_roles_translate_role( $this->name );
