...
class Widget_Users extends \WP_Widget {
	...
	public function __construct() {
		...
		$widget_options = array(
			'classname'   => 'users',
			'description' => esc_html__( 'Provides the ability to list the users of the site.', 'user_roles' )
		);
		...
		$control_options = array(
			'width'   => 525,
			'height'  => 350,
			'id_base' => 'user_roles-widget-users'
		);

		// Create the widget.
		parent::__construct( 'user_roles-widget-users', esc_html__( 'user_roles: Users', 'user_roles' ), $widget_options, $control_options );

		// Set up the defaults.
		$this->defaults = array(
			'title'      => esc_attr__( 'Users', 'user_roles' ),
			'order'      => 'ASC',
			'orderby'    => 'login',
			'role'       => '',
			'meta_key'   => '',
	...
	function update( $new_instance, $old_instance ) {
		...
		// Roles.
		$instance['role'] = user_roles_role_exists( $new_instance['role'] ) ? $new_instance['role'] : '';
	...
	function form( $instance ) {
		...
		$order = array(
			'ASC'  => esc_attr__( 'Ascending',  'user_roles' ),
			'DESC' => esc_attr__( 'Descending', 'user_roles' )
		);

		$orderby = array(
			'display_name' => esc_attr__( 'Display Name', 'user_roles' ),
			'email'        => esc_attr__( 'Email',        'user_roles' ),
			'ID'           => esc_attr__( 'ID',           'user_roles' ),
			'nicename'     => esc_attr__( 'Nice Name',    'user_roles' ),
			'post_count'   => esc_attr__( 'Post Count',   'user_roles' ),
			'registered'   => esc_attr__( 'Registered',   'user_roles' ),
			'url'          => esc_attr__( 'URL',          'user_roles' ),
			'user_login'   => esc_attr__( 'Login',        'user_roles' )
		);

		$meta_key = array_merge( array( '' ), (array) user_roles_get_user_meta_keys() );

		$roles = user_roles_get_roles();
		asort( $roles ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php esc_html_e( 'Order By:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php esc_html_e( 'Order:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'role' ); ?>"><?php esc_html_e( 'Role:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Limit:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php esc_html_e( 'Offset:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'include' ); ?>"><?php esc_html_e( 'Include:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'exclude' ); ?>"><?php esc_html_e( 'Exclude:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'search' ); ?>"><?php esc_html_e( 'Search:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_key' ); ?>"><?php esc_html_e( 'Meta Key:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_value' ); ?>"><?php esc_html_e( 'Meta Value:', 'user_roles' ); ?></label>
