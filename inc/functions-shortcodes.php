...
# Add shortcodes.
add_action( 'init', 'user_roles_register_shortcodes' );
 ...
 * @return void
 */
function user_roles_register_shortcodes() {

	// Add the `[user_roles_login_form]` shortcode.
	add_shortcode( 'user_roles_login_form', 'user_roles_login_form_shortcode' );
	add_shortcode( 'login-form',         'user_roles_login_form_shortcode' ); // @deprecated 1.0.0

	// Add the `[user_roles_access]` shortcode.
	add_shortcode( 'user_roles_access', 'user_roles_access_check_shortcode' );
	add_shortcode( 'access',         'user_roles_access_check_shortcode' ); // @deprecated 1.0.0

	// Add the `[user_roles_feed]` shortcode.
	add_shortcode( 'user_roles_feed', 'user_roles_feed_shortcode' );
	add_shortcode( 'feed',         'user_roles_feed_shortcode' ); // @deprecated 1.0.0

	// Add the `[user_roles_logged_in]` shortcode.
	add_shortcode( 'user_roles_logged_in', 'user_roles_is_user_logged_in_shortcode' );
	add_shortcode( 'is_user_logged_in', 'user_roles_is_user_logged_in_shortcode' ); // @deprecated 1.0.0

	// Add the `[user_roles_not_logged_in]` shortcode.
	add_shortcode( 'user_roles_not_logged_in', 'user_roles_not_logged_in_shortcode' );

	// @deprecated 0.2.0.
	add_shortcode( 'get_avatar', 'user_roles_get_avatar_shortcode' );
	add_shortcode( 'avatar',     'user_roles_get_avatar_shortcode' );
}
 ...
 * @return string
 */
function user_roles_is_user_logged_in_shortcode( $attr, $content = null ) {
 ...
 * @return string
 */
function user_roles_not_logged_in_shortcode( $attr, $content = null ) {
 ...
 * @return string
 */
function user_roles_feed_shortcode( $attr, $content = null ) {
 ...
 * @return string
 */
function user_roles_access_check_shortcode( $attr, $content = null ) {
	...
	$attr = shortcode_atts( $defaults, $attr, 'user_roles_access' );
	...
	if ( $attr['capability'] ) {
		...
		if ( '!' === $operator )
			return user_roles_current_user_can_any( $caps ) ? '' : do_shortcode( $content );
	...
	if ( $attr['role'] ) {
		...
		if ( '!' === $operator )
			return user_roles_current_user_has_role( $roles ) ? '' : do_shortcode( $content );
 ...
 * @return string
 */
function user_roles_login_form_shortcode() {
