...
class Widget_Login extends \WP_Widget {
	...
	public function __construct() {
		...
		$widget_options = array(
			'classname'   => 'login',
			'description' => esc_html__( 'A widget that allows users to log into your site.', 'user_roles' )
		);
		...
		$control_options = array(
			'width'   => 800,
			'height'  => 350,
			'id_base' => 'user_roles-widget-login'
		);

		// Create the widget.
		parent::__construct( 'user_roles-widget-login', esc_attr__( 'user_roles: Login Form', 'user_roles' ), $widget_options, $control_options );

		// Set up the defaults.
		$this->defaults = array(
			'title'           => esc_attr__( 'Log In',      'user_roles' ),
			'label_username'  => esc_attr__( 'Username',    'user_roles' ),
			'label_password'  => esc_attr__( 'Password',    'user_roles' ),
			'label_log_in'    => esc_attr__( 'Log In',      'user_roles' ),
			'label_remember'  => esc_attr__( 'Remember Me', 'user_roles' ),
			'form_id'         => 'loginform',
			...
			'value_username'  => '',
			'show_avatar'     => true,
			'logged_out_text' => esc_html__( 'Please log into the site.',   'user_roles' ),
			'logged_in_text'  => esc_html__( 'You are currently logged in.', 'user_roles' )
		);
	}
	...
	function widget( $sidebar, $instance ) {
		...
		// If the current user is not logged in.
			...
			echo '<div class="user_roles-login-form">' . wp_login_form( $args ) . '</div>';
		}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'label_username' ); ?>"><?php esc_html_e( 'Username Label:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'label_password' ); ?>"><?php esc_html_e( 'Password Label:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'label_log_in' ); ?>"><?php esc_html_e( 'Log In Label:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'label_remember' ); ?>"><?php esc_html_e( 'Remember Me Label:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'value_username' ); ?>"><?php esc_html_e( 'Username Value:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'id_username' ); ?>"><?php esc_html_e( 'Username Field ID:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'id_remember' ); ?>"><?php esc_html_e( 'Remember Me Field ID:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'id_password' ); ?>"><?php esc_html_e( 'Password Field ID:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'id_submit' ); ?>"><?php esc_html_e( 'Submit Button ID:', 'user_roles' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'logged_in_text' ); ?>"><?php esc_html_e( 'Logged in text:', 'user_roles' ); ?></label>
