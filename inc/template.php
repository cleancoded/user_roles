 ...
 * @return bool
 */
function user_roles_can_user_view_post( $user_id, $post_id = '' ) {
	...
	// can roll their own handling of this and filter `user_roles_can_user_view_post`.
	if ( $post instanceof \WP_Post && user_roles_content_permissions_enabled() ) {
		...
		if ( empty( $roles ) )
			$roles = user_roles_convert_old_post_meta( $post_id );
		...
		if ( ! empty( $roles ) && is_array( $roles ) ) {
			...
			// Else, let's check the user's role against the selected roles.
				...
				foreach ( $roles as $role ) {
					...
					if ( user_roles_user_has_role( $user_id, $role ) ) {
						$can_view = true;
						break;
					}
				}
	...
	if ( apply_filters( 'user_roles_check_parent_post_permission', $check_parent, $post_id, $user_id ) ) {
		...
		if ( $post instanceof \WP_Post ) {
			...
			if ( 0 < $parent_id ) {
				$can_view = user_roles_can_user_view_post( $user_id, $parent_id );
			}
		}
	}

	// Allow developers to overwrite the final return value.
	return apply_filters( 'user_roles_can_user_view_post', $can_view, $user_id, $post_id );
}

/**
 * Wrapper function for the user_roles_can_user_view_post() function. This function checks if the
 ...
 * @return bool
 */
function user_roles_can_current_user_view_post( $post_id = '' ) {

	return user_roles_can_user_view_post( get_current_user_id(), $post_id );
}
 ...
 * @return string
 */
function user_roles_list_users( $args = array() ) {
	...
	if ( ! empty( $users ) ) {
		...
		$output = sprintf( '<ul class="xoxo user_roles-list-users">%s</ul>', $output );
	}

	$output = apply_filters( 'user_roles_list_users', $output );
