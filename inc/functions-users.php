...
if ( user_roles_explicitly_deny_caps() ) {
	add_filter( 'user_has_cap', 'user_roles_user_has_cap_filter', 10, 4 );
}
 ...
 * @return array
 */
function user_roles_user_has_cap_filter( $allcaps, $caps, $args, $user ) {
 ...
 * @return bool
 */
function user_roles_user_has_role( $user_id, $roles ) {
 ...
 * @return bool
 */
function user_roles_current_user_has_role( $roles ) {

	return is_user_logged_in() ? user_roles_user_has_role( get_current_user_id(), $roles ) : false;
}
 ...
 * @return bool
 */
function user_roles_current_user_can_any( $caps = array() ) {
 ...
 * @return bool
 */
function user_roles_current_user_can_all( $caps = array() ) {
 ...
 * @return array
 */
function user_roles_get_user_role_names( $user_id ) {
	...
	foreach ( $user->roles as $role )
		$names[ $role ] = user_roles_get_role( $role )->get( 'label' );
